/*
* Ubah waktu reset otomatis menjadi jam 1 pagi
* Dibuat oleh dana putra | Betabotz | aqua bot
* wm ini boleh dihapus bebas kalian juga boleh belajar cara kerja code-nya. Happy coding!:)
...
Contoh:
- *.${command} on* untuk mengaktifkan reset otomatis setiap jam 01:00
- *.${command} off* untuk menonaktifkan reset otomatis`,
    ...
    if (args[0] === 'on') {
        ...
        conn.reply(m.chat, `*Reset limit otomatis akan dijalankan setiap jam 01:00.*`, null);
    } else if (args[0] === 'off') {
        if (!isAutoResetEnabled) {
...
function resetLimit(conn, lim) {
    ...
    conn.reply('120363165389721532@g.us', `*Limit berhasil direset ${lim} / user*`, null); // Kirim info ke grup tertentu
}

function getTimeUntilOneAM() {
    let now = new Date();
    let nextOneAM = new Date(now);
    nextOneAM.setHours(1, 0, 0, 0); 
    if (now.getHours() >= 1) {
        nextOneAM.setDate(nextOneAM.getDate() + 1); // Jika sudah lewat jam 1, set ke hari berikutnya
    }
    return nextOneAM - now;
}

function scheduleDailyReset(conn, lim) {
    let timeUntilOneAM = getTimeUntilOneAM();
