...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    if (text.startsWith('https://nekopoi.care/hentai/')) {
        try {
            const ambil = await (await fetch(`https://api.botcahx.eu.org/api/webzone/nekopoi-detail?query=${text}&apikey=${btc}`)).json();
...
`;
        ...
        try {
            const get2 = await (await fetch(`https://api.botchax.eu.org/api/webzone/nekopoi-detail?query=${text}&apikey=${btc}`)).json();
...
`;
        ...
        const searchResultsMMK = await (await fetch(`https://api.botchax.eu.org/api/webzone/nekopoi-search?query=${text}&apikey=${btc}`)).json();
...
${searchResults.map(v => `*Name:* ${v.title}\n*Link:* ${v.url}\n======================`).join`\n\n`}
    `.trim();
        ...
        let XM4ZE = await (await fetch(`https://api.botcahx.eu.org/api/webzone/nekopoi-update?apikey=${btc}`)).json();
